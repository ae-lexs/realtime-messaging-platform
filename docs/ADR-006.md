...
This section maps REST endpoints to DynamoDB operations from ADR-003, demonstrating that the API contract is directly implementable against the defined data model.

### Endpoint to DynamoDB Table Mapping

| Endpoint | DynamoDB Table | Access Pattern | Key Condition |
|----------|----------------|----------------|---------------|
| `POST /auth/verify-otp` | `users` | Get/Create by phone | GSI: `phone_number-index` → GetItem by `user_id` |
| `GET /users/me` | `users` | Get by ID | PK: `user_id` |
| `PATCH /users/me` | `users` | Update by ID | PK: `user_id` |
| `GET /users/{id}` | `users` | Get by ID | PK: `user_id` |
| `POST /users/lookup` | `users` | Batch get by phone | GSI: `phone_number-index` (batch) |
| `POST /chats` | `chats`, `chat_counters`, `chat_memberships` | TransactWriteItems | Creates chat + counter + memberships atomically |
| `GET /chats` | `chat_memberships` | Query user's chats | GSI: `user_chats-index`, PK: `user_id` |
| `GET /chats/{id}` | `chats`, `chat_memberships` | Get chat + members | PK: `chat_id` (both tables) |
| `PATCH /chats/{id}` | `chats` | Update chat | PK: `chat_id` |
| `POST /chats/{id}/members` | `chat_memberships` | Add member | PutItem with condition |
| `DELETE /chats/{id}/members/{uid}` | `chat_memberships` | Remove member | DeleteItem |
| `GET /chats/{id}/messages` | `messages` | Query by sequence | PK: `chat_id`, SK: `sequence` (range) |
| `GET /chats/{id}/messages/{mid}` | `messages` | Scan or GSI | Requires `message_id` GSI or scan |
| `PATCH /delivery-state/{chat_id}` | `delivery_state` | Update watermark | PK: `user_id`, SK: `chat_id` |
| `GET /sessions` | `sessions` | Query user sessions | GSI: `user_sessions-index` |
| `DELETE /sessions/{id}` | `sessions` | Delete session | PK: `session_id` |

### Query Alignment with DynamoDB Patterns

**Message History (`GET /chats/{id}/messages`):**
- Maps directly to DynamoDB Query: `PK = chat_id, SK > after_sequence` or `SK < before_sequence`
- `direction=backward` uses `ScanIndexForward=false`
- Cursor encodes `{chat_id, sequence}` for efficient continuation
- Strongly consistent read ensures read-your-own-writes

**Chat List (`GET /chats`):**
- Uses GSI `user_chats-index` with `PK = user_id`
- `updated_since` filter applied post-query (DynamoDB doesn't support timestamp filtering on GSI)
- Eventually consistent read acceptable per Section 1.9

**User Lookup (`POST /users/lookup`):**
- BatchGetItem on GSI `phone_number-index`
- Maximum 100 items aligns with DynamoDB BatchGetItem limit
