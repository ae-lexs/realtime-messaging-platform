...
- **Two substrate corrections found while implementing (they change *how*, not *what*):**
  ...
  2. **A Managed Kafka cluster in the same region is a prerequisite for the registry**, so the cluster moves from M2.3 into M0.3 (minimum size: 3 vCPU / 3 GiB, no topics — topics stay in M2.3 with the producer, per Principle 3). This is a sequencing change only: ADR-021 Acceptance Condition §1 (wire format fixed before the cluster exists) is already discharged by ADR-022, and the same PR fixes the format.
