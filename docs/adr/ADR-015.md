...
> [!IMPORTANT]
> **Amended for the GCP substrate (2026-07-26, M1.2 — see the Changelog and Appendix F).** The auth *logic* below is unchanged and was carried over intact; ADR-021 judged it 13/20 substrate-neutral and the re-home confirmed that. What changed is where state and secrets live. Read the body with these four substitutions in mind:
>
> - **DynamoDB → Firestore.** `otp_requests`, `users` and `sessions` are Firestore collections, specified in **[ADR-023](ADR-023.md) v1.2**. `TransactWriteItems` becomes a Firestore `RunTransaction`; the `PHONE#` sentinel of §5.1 becomes the `phone_index/{phone_hash}` document — still required, because Firestore transactions give no phantom protection (ADR-023 v1.2).
> - **Secrets Manager + SSM Parameter Store → GCP Secret Manager** (§3.2, §7). One service, one naming scheme; see the Changelog for the mapping.
> - **Amazon SNS → no SMS provider.** The `SMSProvider` interface (§2.1) is unchanged and `LogSMSProvider` is the only implementation. **`otp_ciphertext` and the KMS-backed same-OTP re-send (§1.1, §1.2) are dropped** — see the Changelog.
> - **ECS task roles → GKE Workload Identity.** Least-privilege key access (§3.2) is enforced by per-service Google service accounts bound to Kubernetes service accounts.
>
> Everything else — OTP generation, the HMAC-with-pepper construction, the state machine, rotation with reuse detection, session limits, revocation, and every invariant in Appendix D — stands as written.

- **Status**: Proposed (amended 2026-07-26 for GCP; v1.2 adds the M1.2 deploy findings)
- **Date**: 2026-02-01
...
> [!IMPORTANT]
> **Superseded by v1.3 (2026-07-26). No SMS is sent, in any environment.** The table below is the AWS-era plan; `SNSProvider` and `FixedProvider` were never built. `LogSMSProvider` is the only implementation and the composition root wires it unconditionally. See "**No SMS delivery**" below for the current decision.
...
**Why SNS over Twilio for MVP** *(historical)*: SNS is native AWS, requires no additional vendor relationship, and integrates with IAM (no API key management). Twilio offers better deliverability and international coverage, but for a lab project with limited SMS volume, SNS is operationally simpler. The provider interface allows swapping to Twilio without code changes outside the provider implementation.

#### 2.2.1 No SMS delivery (v1.3, current)

**Decision: `LogSMSProvider` is the only implementation, and real SMS delivery is an explicit non-goal.**

**The argument that chose SNS now argues against a GCP successor.** SNS won on being *native to the substrate* — no extra vendor, IAM instead of API keys. **GCP has no first-party programmable SMS service**, so that advantage has no counterpart. Two products get mistaken for one: **FCM** is push notifications, not SMS (and push is already a non-goal), and **Cloud Identity Platform** phone sign-in does send SMS but is an *identity product*, not an SMS pipe — Google generates, sends and verifies the code and returns its own token. Adopting it would delete this ADR's subsystem: the pepper, the MAC, `otp_requests`, and `AuthTx`'s OTP condition. That is the right answer for a product that wants phone login and the wrong one for a platform built to own the mechanism.

**What delivery would actually cost.** The code is the cheap part: one adapter behind the unchanged §2.1 interface, an API key as a fifth Secret Manager secret with an accessor binding, and a config switch — §2.1's vendor-independence claim holds exactly as written. The expensive part is **A2P sender registration** (US 10DLC, Mexican sender ID), a business-verification process measured in weeks, for a deploy-and-destroy lab with no users to receive a message.

**The abuse surface is already handled.** An unauthenticated OTP endpoint with a live provider is the target of SMS-pumping fraud. ADR-013 §4.1's limits — 3 per phone and 10 per IP per 15 minutes, plus the lockout — are implemented and were verified live in the M1.2 flow gate. This is the one part of delivery that is ready.

**The condition on reversing this.** `LogSMSProvider` logs the OTP in full (phone masked, code not) into Cloud Logging. That is why the M1.2 gate can read it, and it is an account takeover the moment a real user exists. **Wiring a live provider must remove `LogSMSProvider` from deployed environments in the same change** — not afterwards.
...
| DynamoDB TransactionCanceledException (phone sentinel) | Concurrent registration race | ~~Query phone_number-index → proceed as existing user~~ **Corrected in v1.4: return 401 INVALID_OTP.** The winner consumed the OTP inside its own transaction, so the loser holds a spent code and has no login to proceed to; the client requests a fresh OTP and signs in to the account the race created | Deny |
...
| **ADR-021** | GCP substrate migration — the amendment banner at the top of this record |
| **ADR-023** | Two-store data model; **v1.2 specifies `otp_requests` and `phone_index`**, the tables this ADR introduced |

---

## Appendix F: GCP substrate mapping (2026-07-26, M1.2)

### Secret storage

AWS split this across two services; GCP does not, so the two-tier naming collapses into one. Secret Manager has no free-form hierarchy — a secret ID is a flat name — so the SSM path segments become underscore-separated components.

| §3.2 (AWS) | GCP Secret Manager | Notes |
|---|---|---|
| Secrets Manager `jwt/signing-key/{KEY_ID}` | `jwt-signing-key-{KEY_ID}` | RSA-2048 private key, PEM |
| SSM `/messaging/jwt/public-keys/{KEY_ID}` | `jwt-public-key-{KEY_ID}` | PEM; listed by prefix at startup to build the `kid` → key map |
| SSM `/messaging/jwt/current-key-id` | `jwt-current-key-id` | Names the active signing key |
| Secrets Manager `{prefix}/otp/pepper` | `otp-pepper` | HMAC pepper (§1.2) |

**Provisioning split.** Terraform creates the *secrets*; it never creates *versions*. Key material must not enter Terraform state, so `scripts/auth-keys.sh` generates the key pair and pepper and adds the versions imperatively — the same split M0.3 used for the schema registry, and for the same reason: state should not claim what it should not hold.

**Access control.** §3.2's least-privilege table is unchanged in intent. ECS task roles become a Google service account per service, bound to the Kubernetes service account via GKE Workload Identity: ChatMgmt gets `roles/secretmanager.secretAccessor` (private key + pepper) and `roles/datastore.user`; Gateway and Ingest get accessor scoped to the public-key secrets; Fanout gets neither.

### Dropped: `otp_ciphertext` and the same-OTP re-send

§1.1 stores `otp_ciphertext = KMS.Encrypt(otp)` so that a repeat `request-otp` inside the 5-minute window can decrypt and re-deliver the *same* OTP rather than telling the user to wait, and §1.2's `ConditionalCheckFailed` branch is written around it. **This is dropped, not deferred.**

- **It was never implemented.** The merged v1 code carries the field on the record struct and never populates it; the `ErrAlreadyExists` branch already returns the existing `expires_at` and `retry_after_seconds` and sends nothing. The amendment records what the code does instead of leaving a field that reads as implemented.
- **The cost is real and the benefit is not.** Re-send would require a Cloud KMS keyring, a crypto key, an IAM binding, and an encrypt call on the OTP path — for a lab whose only SMS provider writes a log line. There is no carrier to be out of sync with.
- **The invariant it protected still holds.** §1.2's argument for re-sending was that generating a *new* OTP would invalidate an in-flight SMS. Not re-sending at all preserves that property just as well: the stored OTP is untouched, and the original remains the valid one until it expires.

The `SMSProvider` interface (§2.1) is unchanged, so a real provider can be added later without touching auth logic; re-send would return with it if it is ever wanted.

### Keys are resolved by name, never enumerated (M1.2 deploy finding)

§3.2 discovers the set of acceptable `kid` values by enumerating published public keys. On GCP that is **ungrantable at the right scope**, and the first deploy is what proved it: `secretmanager.secrets.list` is a **project-level** permission — listing is a query over the project's whole secret collection — so the per-secret `secretAccessor` bindings above can never satisfy it. The pod crash-looped on `PermissionDenied`.

Supporting enumeration would have meant granting `roles/secretmanager.viewer` project-wide, widening Chat Mgmt's reach from four named secrets to the metadata of every secret in the project — a permanent cost, paid for a rotation path this record had already deferred. Keys are therefore resolved **by name**: `jwt-current-key-id` names the active key, and the signing and public secrets are named after it.

**The trade, stated plainly:** exactly one `kid` is accepted at a time, so a rotation invalidates access tokens minted under the previous key. They live at most an hour (§3.3), so the cost is up to an hour of forced re-auth during a deliberate rotation. If multi-key verification is ever wanted, the answer is a configured list of accepted key IDs — not enumeration.

**What did work first time:** GKE Workload Identity. The pod authenticated to Secret Manager as its own service account with no key file; the failure was one specific missing permission, not a missing identity.

### Deferred: unknown-`kid` immediate refresh

§3.2 specifies that a JWT bearing an unknown `kid` triggers a single out-of-cache secret refresh with a 30-second cooldown. `RemoteKeyStore` implements startup load plus the 5-minute periodic refresh; the unknown-`kid` path is **not** implemented. It was not implemented on AWS either — the v1 code shipped only the `KeyStore` interface and `StaticKeyStore` — so building it now would be new scope in a re-home PR. It matters only during a key rotation, which nothing exercises until the M7 verification module.
...
|---|---|---|
| v1.4 | 2026-07-26 | **The registration-race recovery in §10.2 was never reachable; corrected to a refusal.** The row said a caller losing the phone sentinel should "query phone_number-index → proceed as existing user", and the M1.2 implementation did exactly that. It cannot work, on either substrate: the winner's transaction consumes the OTP as part of the same all-or-nothing write set (§5.1), so the loser's follow-up login presents a code whose status is already `verified` — and the login transaction asserts `status = pending`, by design, because a code is consumed once. The redirect therefore always ended in `INVALID_OTP` one round trip later than necessary, and the app-layer test that claimed otherwise passed only because its fake transactor accepted a consumed OTP. Corrected in both directions: the caller now refuses immediately with 401 INVALID_OTP and the client requests a fresh code, which signs it in to the account the race created. **No invariant changes** — one_code_one_session and phone uniqueness are exactly what made the redirect impossible. Recorded rather than quietly patched because §10.2 is the contract a client integrator reads. |
| v1.3 | 2026-07-26 | **SMS delivery recorded as an explicit non-goal (§2.2.1).** v1.1's banner said "Amazon SNS → no SMS provider" while §2.2's table still listed `SNSProvider` as Production, so the record contradicted itself and a reader landing on §2.2 got the AWS answer. §2.2 is now banner-marked historical and §2.2.1 states the decision: **`LogSMSProvider` is the only implementation, in every environment**; `SNSProvider` and `FixedProvider` were never built. The reasoning is that §2.2's own criterion — native to the substrate, no extra vendor — has **no GCP counterpart, because GCP has no first-party programmable SMS service**; FCM is push, and Cloud Identity Platform phone sign-in is an identity product that would replace this ADR's OTP subsystem rather than serve it. Delivery would be a small adapter behind the unchanged §2.1 interface and a large amount of A2P sender registration, for a lab with no users. Records that the abuse surface is already covered (ADR-013 §4.1 limits, verified live in the M1.2 gate) and the condition on reversing: **`LogSMSProvider` logs the OTP in full and must be removed from deployed environments in the same change that wires a live provider.** |
| v1.2 | 2026-07-26 | **Deploy findings (M1.2), no decision reversed.** Two corrections found by running against the live project. (1) **Keys are resolved by name, never enumerated** — §3.2's discovery of acceptable `kid` values by listing published public keys is ungrantable on GCP, because `secretmanager.secrets.list` is a project-level permission that per-secret `secretAccessor` bindings cannot satisfy; supporting it meant `roles/secretmanager.viewer` project-wide, for a rotation path v1.1 had already deferred. The trade — one accepted `kid`, so a rotation forces re-auth bounded by the 1 h access-token lifetime — is recorded in Appendix F. Also recorded there: **Workload Identity worked on the first deploy**; the pod authenticated as its own service account and the failure was one missing permission, not a missing identity. (2) **§1.2's conditional put has three disjuncts and only two were implemented** — `#status = "verified"` was dropped, so a consumed OTP blocked reissue for the rest of its five-minute window and a user who had just logged in could not request another code. The record was right and the implementation was wrong; nothing is protected by holding a spent OTP, since it cannot be replayed and throttling re-requests belongs to ADR-013 §4.1's rate limiter. |
| v1.1 | 2026-07-26 | **GCP substrate amendment (M1.2). No change to any auth decision, control, or invariant.** The logic in this record was carried over intact and its Appendix D invariants all still hold; only substrate bindings moved. Added the amendment banner and **Appendix F**, which specifies: the Secrets Manager + SSM → **GCP Secret Manager** mapping (two services collapse to one; SSM paths become flat underscore-separated IDs) and the Terraform-creates-secrets / script-creates-versions split that keeps key material out of state; ECS task roles → **GKE Workload Identity** for §3.2's least-privilege key access; DynamoDB → **Firestore** with `TransactWriteItems` → `RunTransaction` and the §5.1 `PHONE#` sentinel → `phone_index/{phone_hash}` (still required — Firestore transactions offer no phantom protection, ADR-023 v1.2). Two scope calls recorded honestly rather than silently: **`otp_ciphertext` and the KMS-backed same-OTP re-send are dropped** (never implemented, would need Cloud KMS on the OTP path, and the in-flight-SMS property it protected holds anyway when nothing is re-sent), and the **unknown-`kid` immediate refresh is deferred** (also never implemented on AWS; only reachable during a rotation, which nothing exercises before M7). |
| v1.0 | 2026-02-01 | Initial version (AWS substrate). OTP storage (`otp_requests` + Redis counters), HMAC-with-pepper OTP construction, provider-interface SMS delivery, token minting in `internal/auth`, refresh rotation with reuse detection, session limits, revocation, and the Appendix D invariants. |
