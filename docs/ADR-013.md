...
flowchart TB
    ...
    subgraph Surfaces["ATTACK SURFACES"]
        WS["WebSocket Gateway<br/>• Connection flooding<br/>• Protocol abuse<br/>• Token theft"]
        REST["REST API<br/>• Enumeration<br/>• Brute force<br/>• Injection"]
        MSG["Message Path<br/>• Spam/flooding<br/>• Content abuse<br/>• Amplification"]
        DATA["Data Stores<br/>• Unauthorized access<br/>• Data exfiltration<br/>• Corruption"]
    end
    
    subgraph Assets["PROTECTED ASSETS"]
        USER["User Data<br/>• Phone numbers<br/>• Messages<br/>• Memberships"]
        SYS["System Integrity<br/>• Message ordering<br/>• Delivery guarantees<br/>• Availability"]
        PRIV["Privacy<br/>• Presence<br/>• Relationships<br/>• Communication patterns"]
    end
...
flowchart TB
    subgraph Limits["CONNECTION RATE LIMITS"]
        ...
        IP["Per-IP Limits"]
        IP1["• 10 connections/sec"]
        IP2["• 100 connections/minute"]
        IP3["• Shared across all IPs in /24"]
        
        USER["Per-User Limits"]
        USER1["• 5 concurrent connections"]
        USER2["• 10 new connections/minute"]
        USER3["• Applied after authentication"]
        
        GLOBAL["Global Limits"]
        GLOBAL1["• 10K connections/sec cluster-wide"]
        GLOBAL2["• Circuit breaker at 80% capacity"]
...
flowchart TB
    subgraph Lifecycle["TOKEN LIFECYCLE"]
        ISSUE["Token Issued<br/>• OTP verification<br/>• Token refresh"]
        ACTIVE["Token Active<br/>• Used for auth<br/>• Validated each request"]
        REFRESH["Token Refreshed<br/>• New access token<br/>• New refresh token"]
        REVOKE["Token Revoked<br/>• Explicit logout<br/>• Session deletion<br/>• Security event"]
        EXPIRE["Token Expired<br/>• TTL exceeded<br/>• Natural expiration"]
    end
...
flowchart TB
    subgraph Limits["MESSAGE RATE LIMITS"]
        ...
        PC1["• 10 msg/sec sustained"]
        PC2["• 20 msg burst (1 sec)"]
        PC3["• Applies to sender in chat"]
        
        PERUSER["Per-User Limits"]
        PU1["• 30 msg/sec total"]
        PU2["• 1000 msg/hour total"]
        PU3["• Across all chats"]
        
        GLOBAL["Global Limits"]
        GL1["• 100K msg/sec cluster"]
        GL2["• Backpressure at 80%"]
...
flowchart TB
    subgraph Events["SECURITY EVENT TAXONOMY"]
        ...
        AUTH1["• auth.otp_requested"]
        AUTH2["• auth.otp_verified"]
        AUTH3["• auth.otp_failed"]
        AUTH4["• auth.token_refreshed"]
        AUTH5["• auth.logout"]
        
        SESSION["Session Events"]
        SESS1["• session.created"]
        SESS2["• session.revoked"]
        SESS3["• session.expired"]
        SESS4["• session.anomaly_detected"]
        
        ACCESS["Access Events"]
        ACC1["• access.denied"]
        ACC2["• access.rate_limited"]
        ACC3["• access.membership_violation"]
        
        ABUSE["Abuse Events"]
        ABUSE1["• abuse.spam_detected"]
        ABUSE2["• abuse.flooding_detected"]
        ABUSE3["• abuse.enumeration_suspected"]
...
flowchart TB
    subgraph Response["INCIDENT RESPONSE"]
        DETECT["Detection<br/>• Alerts<br/>• User report<br/>• External report"]
        TRIAGE["Triage<br/>• Assess severity<br/>• Assign responder"]
        CONTAIN["Containment<br/>• Isolate affected systems<br/>• Revoke compromised creds"]
        MITIGATE["Mitigation<br/>• Deploy fix<br/>• Verify resolution"]
        RECOVER["Recovery<br/>• Restore normal ops<br/>• Monitor for recurrence"]
        REVIEW["Post-Incident<br/>• Root cause analysis<br/>• Process improvements"]
