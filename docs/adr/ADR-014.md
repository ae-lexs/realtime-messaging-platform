...
> [!WARNING]
> **Superseded by [ADR-021](ADR-021.md)** (2026-07-24, AWS → GCP substrate migration). The entire AWS substrate below — DynamoDB, MSK Serverless, ElastiCache, ECS Fargate, ALB, CloudWatch/X-Ray, and the LocalStack dev stack — is replaced by the GCP substrate in ADR-021 (Cloud SQL + Firestore, Managed Kafka, Memorystore, GKE Autopilot, Managed Prometheus/Cloud Trace). The Docker toolchain is retained; local cloud-emulation is removed. This record is retained unchanged.
...
7. **Docker as the Sole Development Dependency**: A contributor should be able to clone the repository and run every build, test, lint, and code generation command with only Docker and Make installed. No local Go toolchain, `buf`, `golangci-lint`, `terraform`, or any other tool installation is required. This eliminates version skew between contributors, guarantees CI/local parity (the same container that runs `make lint` locally runs `make lint` in CI), and reduces onboarding friction to a single prerequisite. A local Go installation is useful for IDE support (`gopls`), but it is strictly optional — it is never invoked by any Makefile target.

8. **Escape Hatches**: Choices should avoid deep vendor lock-in where alternatives exist. ADR-012's OpenTelemetry abstraction is the model: define contracts against open standards, implement with pragmatic vendor choices.
...
|----------------|---------------|---------------|
| Language | Go 1.25+ | Goroutine concurrency, single-binary, first-class gRPC |
...
| Service Architecture | Clean Architecture (port/app/adapter) | Enforced boundaries, testable layers, DDD-lite domain modeling |
| Development Toolchain | Docker-only (toolbox container) | Sole dependency is Docker + Make; zero host-installed tools; CI/local parity |
...
│   └── messaging/v1/
├── test/                       # Test infrastructure (ADR-017)
│   ├── harness/                # Reference client + test infrastructure
...
├── docker/                     # Dockerfiles
│   ├── dev.Dockerfile          # Multi-stage: base → dev (Go + tools + Air) → build → prod
│   ├── gateway.Dockerfile      # Production: builder → scratch
│   ├── ingest.Dockerfile
│   ├── fanout.Dockerfile
│   └── chatmgmt.Dockerfile
├── docker-compose.yaml         # Infrastructure: LocalStack, Redpanda, Redis
├── docker-compose.dev.yaml     # Override: 4 services with Air hot reload + toolbox
├── .air/                       # Per-service Air hot-reload configurations
│   ├── gateway.toml
│   ├── ingest.toml
│   ├── fanout.toml
│   └── chatmgmt.toml
...
├── .arch-go.yml                # Architectural boundary rules (Clean Architecture)
├── Makefile                    # Developer ergonomics — all targets delegate to Docker
├── go.mod                      # Single Go module
├── go.sum
├── CONTRIBUTING.md             # Code standards, workflow, conventions
└── README.md                   # Architecture overview, getting started, Makefile reference
```
...
**Clean Architecture per Service (`port/app/adapter`)**: Each service directory follows the Three Dots Labs interpretation of Clean Architecture with three layers. `port/` contains entry points (HTTP handlers, gRPC servers, Kafka consumer entrypoints), `app/` contains use cases and orchestration logic, and `adapter/` contains I/O implementations (DynamoDB clients, Kafka producers, Redis operations). Dependencies flow inward only: `port → app → domain`, `adapter → app → domain`. These boundaries are enforced in CI by `go-arch-lint` (structural) and `depguard` (import restrictions). See CONTRIBUTING.md §Clean Architecture for layer definitions, interface placement, and dependency rules.
...
**`test/` as Isolated Test Infrastructure (ADR-017)**: The test harness, conformance suite, contract tests, end-to-end scenarios, and chaos tests live in a dedicated top-level `test/` directory — separate from `internal/` — because they test *the system*, not internal packages. The test harness depends only on public contracts (WebSocket, REST, Prometheus endpoints) and never imports internal service code. See ADR-017 for the L1–L4 test pyramid.
...
**`docker-compose.dev.yaml` as Development Overlay**: The base `docker-compose.yaml` defines infrastructure services (LocalStack, Redpanda, Redis). The `docker-compose.dev.yaml` overlay adds the four application services with Air hot reload and a `toolbox` service containing all development tools. This separation means `make up` starts only infrastructure (useful for running services directly during debugging), while `make dev` starts the full stack. See §4A for the toolbox service design.

**`CONTRIBUTING.md` as Operational Contract**: The CONTRIBUTING guide codifies code standards, development workflow, and architectural conventions — each traceable to an ADR. It exists as a separate document because README.md serves as a project overview and getting started guide, while CONTRIBUTING.md is the normative reference for how code is written, reviewed, and merged. Both are listed in the directory layout to emphasize that they are project deliverables, not afterthoughts.
...
**Critical design principle**: Every Makefile target delegates to a Docker container. No target invokes `go`, `buf`, `golangci-lint`, `terraform`, or any other tool directly on the host. This is the mechanism that enforces Decision Driver #7 (Docker as the Sole Development Dependency). A contributor with only Docker and Make installed can run every target below.

```makefile
.PHONY: help up down dev dev-down proto proto-lint proto-breaking \
        lint lint-fix fmt tidy arch-lint test test-integration \
        test-coverage build docker terraform-fmt terraform-validate ci-local
...
proto:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox sh -c "cd proto && buf generate"

proto-lint:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox sh -c "cd proto && buf lint"

proto-breaking:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox sh -c "cd proto && buf breaking --against '../.git#branch=main,subdir=proto'"

# --- Go tooling ---
lint:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox golangci-lint run ./...

lint-fix:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox golangci-lint run --fix ./...

fmt:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox gofmt -w .

tidy:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox go mod tidy

arch-lint:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox go-arch-lint check

# --- Testing ---
test:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox go test -race -cover ./...

test-integration:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox go test -race -tags=integration ./...

test-coverage:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox sh -c "go test -race -coverprofile=coverage.out ./... && go tool cover -html=coverage.out -o coverage.html"

# --- Build ---
build:
	docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
	  run --rm toolbox go build ./cmd/...
...
# --- Terraform ---
terraform-fmt:
	docker run --rm -v "$$(pwd)/terraform:/terraform" -w /terraform \
	  hashicorp/terraform fmt -recursive
...
**Key observations**:
- Every `go`, `buf`, and `golangci-lint` invocation runs inside the `toolbox` service via `docker compose run --rm`.
- `ci-local` runs the same stages in the same order as the GitHub Actions CI workflow (§8.2), ensuring local/CI parity.
- Terraform targets use the official `hashicorp/terraform` image — no local Terraform installation needed.
- `make dev` composes both YAML files, starting infrastructure + application services + toolbox in one command.

---

## Part 4A: Docker-Only Development Toolchain

### 4A.1 Design Intent

The project's sole required development dependency is Docker (plus Make as a convenience layer). This is a deliberate architectural choice, not a convention of convenience. The rationale:

1. **Version pinning without coordination**: The `dev.Dockerfile` pins exact versions of Go, `golangci-lint`, `buf`, `go-arch-lint`, Delve, and Air. Every contributor runs identical tool versions regardless of their local setup. Version upgrades are a single Dockerfile change, reviewed as a PR.

2. **CI/local equivalence**: The same Docker image that runs `make lint` on a contributor's laptop runs `make lint` in GitHub Actions. "Works on my machine" is structurally impossible — there is no "my machine" in the build path.

3. **Zero onboarding friction**: A new contributor clones the repository, runs `make dev`, and has a fully functional development environment. No `brew install`, no `go install`, no version manager, no PATH configuration.

4. **Optional Go for IDE support**: A local Go 1.25+ installation enables `gopls` for autocomplete, jump-to-definition, and inline diagnostics in VSCode/GoLand. This is explicitly optional — it improves the editing experience but is never invoked by any Makefile target, CI step, or build process.

### 4A.2 Toolbox Service

The `docker-compose.dev.yaml` overlay defines a `toolbox` service that contains the complete Go development environment:

```yaml
# docker-compose.dev.yaml (excerpt)
services:
  toolbox:
    build:
      context: .
...
**Named Docker volumes** (`go-mod-cache`, `go-build-cache`) persist the Go module cache and build cache between container runs. This eliminates cold-start penalties — after the first `make build`, incremental builds complete in sub-second time because the compilation cache persists across `docker compose run` invocations.

### 4A.3 Dev Dockerfile (Multi-Stage)

The `docker/dev.Dockerfile` is a multi-stage build that produces both development and production images from a single file:

```dockerfile
# Stage 1: Base — Go toolchain
FROM golang:1.25-alpine AS base
RUN apk add --no-cache ca-certificates tzdata git

# Stage 2: Dev — all development tools
FROM base AS dev
...
# Stage 3: Build — compile production binaries
...
The `dev` target is what the `toolbox` service uses. The `prod` target is what per-service production Dockerfiles derive from. This ensures tool versions are defined once and reused consistently.

### 4A.4 Hot Reload (Air)

`make dev` starts each service with Air file watching. Per-service configurations live in `.air/`:
...
Air watches `.go` files, excludes `_test.go` and `gen/`, and rebuilds the relevant `cmd/` binary on change. Combined with the named Docker volumes for build cache, this enables sub-second incremental rebuilds: edit a file, save, and see the service restart with the new code within ~1 second.

### 4A.5 Debugging

Delve is available inside the dev containers on port 2345. Connect your IDE debugger to `localhost:2345` (adjust per service if running multiple simultaneously). To start a service under Delve instead of Air:

```bash
docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
  run --rm -p 2345:2345 toolbox \
  dlv debug ./cmd/gateway --headless --listen=:2345 --api-version=2
```

### 4A.6 Ad-Hoc Commands

Any Go tooling command can be run inside the toolbox without modifying the Makefile:
...
This provides escape-hatch access to the full Go toolchain without requiring a local installation.
...
The primary workflow uses Docker exclusively (§4A). No local Go toolchain is required.

```bash
# Start infrastructure only (LocalStack, Redpanda, Redis)
make up

# Start infrastructure + all 4 services with Air hot reload + toolbox
make dev

# Run the full CI pipeline locally (identical stages to GitHub Actions)
make ci-local

# Run an ad-hoc command inside the toolbox
docker compose -f docker-compose.yaml -f docker-compose.dev.yaml \
  run --rm toolbox "go doc ./internal/domain"
...
**Optional local Go workflow**: If a local Go 1.25+ installation is present (for `gopls` IDE support), services *can* be run directly for debugging scenarios. This bypasses Air hot reload and Docker networking but provides faster debugger attachment:
...
ENVIRONMENT=local go run ./cmd/gateway
ENVIRONMENT=local go run ./cmd/ingest
ENVIRONMENT=local go run ./cmd/fanout
ENVIRONMENT=local go run ./cmd/chatmgmt
```

This workflow is explicitly secondary — it exists for debugging convenience, not as the standard development path. All lint, test, build, and proto commands must still go through the Makefile (which delegates to Docker).
...
6. **Docker-Only Onboarding**: A contributor needs only Docker and Make to clone, build, test, lint, and run the entire project. No Go toolchain, `buf`, `golangci-lint`, or `terraform` installation is required. Local Go is optional for IDE support (`gopls`) but is never invoked by any Makefile target. This eliminates version skew between contributors, guarantees CI/local parity, and reduces onboarding to a single prerequisite. See §4A for the complete toolbox design.

7. **CI Guardrails**: `buf breaking` prevents wire-incompatible proto changes; `golangci-lint` catches common Go errors; `-race` flag in tests detects concurrency bugs. The ADR-011 partitioning test vectors are enforced as release-blocking unit tests. `go-arch-lint` and `depguard` enforce Clean Architecture layer boundaries (CONTRIBUTING.md §Clean Architecture).
...
1. **Docker-Only Toolchain Tests** (§4A):
   - Fresh clone on a machine with only Docker 24+ and Make: `make ci-local` completes successfully
   - No Makefile target invokes `go`, `buf`, `golangci-lint`, or `terraform` on the host
   - `make dev` starts infrastructure + all 4 services with hot reload from a cold start
   - Named Docker volumes (`go-mod-cache`, `go-build-cache`) persist between `docker compose run` invocations
   - Toolbox service contains all pinned tool versions from `dev.Dockerfile`

2. **Repository Structure Tests**:
   - All four binaries compile: `make build` (delegated to Docker)
   - `internal/` packages are not importable from outside the module
   - `buf lint` and `buf generate` succeed with no warnings (via `make proto-lint` and `make proto`)
   - Clean Architecture boundaries enforced: `make arch-lint` passes with no violations
   - `depguard` restricts infrastructure library imports to their designated adapter packages

3. **Docker Build Tests**:
   - All four production Dockerfiles produce images based on `scratch`
...
4. **Local Development Tests**:
   - `make up` provisions DynamoDB tables, Kafka topics, and Redis
   - `make dev` starts all four services and connects to local infrastructure
   - End-to-end message flow works locally: send via WebSocket → persist → fanout → deliver

5. **MSK Serverless Verification**:
...
6. **Local Parity Gap Validation** (in dev environment):
...
7. **CI Pipeline Tests**:
   ...
   - `go-arch-lint` catches a deliberately introduced layer boundary violation
   ...
   - `make ci-local` runs identical stages in identical order to GitHub Actions CI workflow

8. **grpc-gateway Verification**:
...
| ADR-017 | Test pyramid: L1–L4 test infrastructure in `test/` directory; conformance suite as CI gate |

**Related Documents** (not ADRs):

| Document | Relationship |
|----------|--------------|
| README.md | Project overview, architecture summary, getting started, Makefile reference. Reflects the Docker-only prerequisite (§4A) and Clean Architecture repo structure (§3.1). |
| CONTRIBUTING.md | Code standards, development workflow, Clean Architecture conventions, domain modeling (DDD Lite), testing standards, commit conventions, Terraform standards, CI/CD pipeline. Normative reference for how code is written and reviewed. |
...
All tool versions below are pinned in `docker/dev.Dockerfile` (development tools) or `docker-compose.yaml` (infrastructure). No versions are resolved at runtime or left to host-installed defaults.

| Technology | Minimum Version | Pinned In | Rationale |
|------------|----------------|-----------|-----------|
| Go | 1.22 | `dev.Dockerfile` base image | Required for `net/http` routing enhancements, `slices` package in stdlib |
| buf CLI | 1.28+ | `dev.Dockerfile` `go install` | Required for `v2` buf.yaml and buf.gen.yaml format |
| golangci-lint | 1.55+ | `dev.Dockerfile` `go install` | Required for Go 1.25 compatibility, v2 config format |
| go-arch-lint | latest stable | `dev.Dockerfile` `go install` | Clean Architecture boundary enforcement |
| Air | latest stable | `dev.Dockerfile` `go install` | Hot reload for development; per-service configs in `.air/` |
| Delve | latest stable | `dev.Dockerfile` `go install` | In-container debugging on port 2345 |
| Terraform | 1.5+ | `hashicorp/terraform` Docker image | Required for `import` blocks and `check` blocks for drift detection |
| Docker | 24+ | Host prerequisite | Required for BuildKit multi-stage build optimizations |
| docker-compose | 2.20+ | Bundled with Docker 24+ | Required for `depends_on.condition` with health checks |
| Redpanda | 23.2+ | `docker-compose.yaml` image tag | Required for full Kafka protocol compatibility (KIP-848 consumer group) |
...
INVARIANT docker_only_toolchain:
  ∀ Makefile target T:
    T.invokes_host_tool ∈ {"docker", "docker compose", "make"} ∨ T.is_echo = true
    ∧ ¬∃ host invocation of {"go", "buf", "golangci-lint", "terraform", "air", "dlv"}
  -- No Makefile target invokes tools on the host; all delegate to Docker containers
  -- A local Go installation is permitted for IDE support but never used by build/test/lint

INVARIANT clean_architecture_layers:
  ∀ service S ∈ {gateway, ingest, fanout, chatmgmt}:
    S.layers = {"port", "app", "adapter"}
    ∧ ¬∃ import from domain → {app, port, adapter}
    ∧ ¬∃ import from app → {port, adapter}
    ∧ ¬∃ import from port → adapter
  -- Clean Architecture dependency rule enforced by go-arch-lint + depguard in CI
