...
> [!WARNING]
> **Superseded by [ADR-021](ADR-021.md)** (2026-07-24, AWS → GCP substrate migration). The sequencing decision changes from **Option 1** (DynamoDB atomic counter, two-phase) to **Option 5** (PostgreSQL sequences), which collapses the two phases into a single transaction. This record is retained unchanged; the design below reflects the AWS-era decision.
