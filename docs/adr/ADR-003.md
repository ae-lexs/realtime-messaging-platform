...
> [!NOTE]
> **Amended by [ADR-021](ADR-021.md)** (2026-07-24, AWS → GCP substrate migration). The authoritative store changes from DynamoDB to **Cloud SQL for PostgreSQL** (message/delivery path) plus **Firestore** (identity/membership); the hierarchy becomes Postgres/Firestore → Kafka → Redis. BigQuery is added as an analytical consumer. The "single complete historical source" invariant is **narrowed** — see ADR-021 (Data Lake §2) for the honest scope and its coverage gaps. The dataflow contracts below otherwise stand.
...
> **Note**: This ADR focuses on **source of truth hierarchy, dataflow contracts, and consistency boundaries**. For complete DynamoDB schema specifications, index strategies, and capacity planning, see **ADR-007: Data Model and Index Strategy**.
...
> **Complete Schema Reference**: For detailed table schemas, attribute definitions, GSI configurations, access patterns, and capacity planning, see **[ADR-007: Data Model and Index Strategy](./ADR-007.md)**.

DynamoDB is the single source of truth for all persistent entity state. The system uses **8 DynamoDB tables** organized by access pattern similarity and scaling characteristics:

| Table | Purpose | Key Structure |
|-------|---------|---------------|
| `users` | User identity and profiles | PK: `user_id` |
| `chats` | Chat metadata | PK: `chat_id` |
| `chat_memberships` | User-chat relationships | PK: `chat_id`, SK: `user_id` |
| `messages` | Message content | PK: `chat_id`, SK: `sequence` |
| `chat_counters` | Atomic sequence counters | PK: `chat_id` |
| `idempotency_keys` | Duplicate detection | PK: `chat_id`, SK: `client_message_id` |
| `delivery_state` | Delivery watermarks | PK: `user_id`, SK: `chat_id` |
| `sessions` | Authentication sessions | PK: `session_id` |

**Critical Invariant**: A user can send messages to a chat if and only if a membership record exists in `chat_memberships`. The Durability Plane checks this on every message ingest using strongly consistent reads.
...
## Appendix A: DynamoDB Schema Summary

> **Complete Reference**: See **[ADR-007: Data Model and Index Strategy](./ADR-007.md)** for complete table schemas, GSI configurations, access pattern mappings, and capacity planning guidelines.
