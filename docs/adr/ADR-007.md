...
> [!WARNING]
> **Superseded by [ADR-021](ADR-021.md)** (2026-07-24, AWS → GCP substrate migration). The single-table DynamoDB model is replaced by a **two-store model** — PostgreSQL tables for the message/delivery write path (`chat_counters`, `messages`, `idempotency_keys`, `delivery_state`) and Firestore document collections for identity/membership (`users`, `chats`, `memberships`, `sessions`). A follow-up amendment will specify the concrete schema. This record is retained unchanged.
