...
4. **Optional Go for IDE support**: A local Go 1.22+ installation enables `gopls` for autocomplete, jump-to-definition, and inline diagnostics in VSCode/GoLand. This is explicitly optional — it improves the editing experience but is never invoked by any Makefile target, CI step, or build process.
...
**Optional local Go workflow**: If a local Go 1.22+ installation is present (for `gopls` IDE support), services *can* be run directly for debugging scenarios. This bypasses Air hot reload and Docker networking but provides faster debugger attachment:
...
| golangci-lint | 1.55+ | `dev.Dockerfile` `go install` | Required for Go 1.22 compatibility, v2 config format |
