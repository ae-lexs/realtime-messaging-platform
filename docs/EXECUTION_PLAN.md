# Execution Plan — Realtime Messaging Platform (v2.7, GCP substrate)

- **Status**: Active
- **Created**: 2026-02-01
- **Re-split**: 2026-07-24 (v2.0 — modules → granular PRs on the GCP substrate)
- **Last amended**: 2026-08-10 (v2.7 — M1.3 premise measured, entity events moved to M2.3; see Document Revision History)
...
- **Four scope decisions settled here** (each recorded in the ADR it amends, not only in this plan):
  ...
  2. **The `PHONE#` uniqueness sentinel survives as `phone_index/{phone_hash}`.** `tx.Create` restores `attribute_not_exists`. *(Reason corrected 2026-08-10: this bullet argued that "a query matching nothing locks nothing, so concurrent first-time registrations would both commit" — measured false, RTM-04 C1 withdrawn. The sentinel is kept because the protection is unspecified and absent from the mobile/web SDKs, not because the race is open; and on this path the OTP document, not the sentinel, is what refuses the losers — C7. ADR-023 v1.4 has the corrected statement.)*
...
- **Delivers:** `internal/chatmgmt` REST/gRPC — create chat (direct dedup + group), add/remove/leave member, role change, mute, list chats, get chat; writes `chats`/`memberships` to Firestore.
- **Scope decision (2026-08-10): entity-event publication moves to M2.3.** M1.3 was to publish `chats.created` / `memberships.changed` at-least-once (ADR-022 D4), but no Kafka **producer** exists in the repo — M2.3 builds the first one for the outbox relay, and the wire format (`internal/events` serde, both event protos, the registry client) is already in place from M0.3. Building a second producer here to retire it there helps nobody, and the obligation M1.3 owes the critical path is membership **reads** (M2.2 validates against Firestore), which it keeps. M2.3 gains the entity-event half, where the producer it needs already lives.
- **ADRs:** ADR-016 (chat lifecycle, §3.1–§3.3 superseded), ADR-006 (chat REST), ADR-023 v1.4 (`chats`, `memberships`, `direct_chats`).
- **Key invariants:** at most one direct chat per pair under concurrency — `tx.Create` on the **`direct_chats/{min}__{max}` sentinel** inside the creation transaction, *not* a deterministic chat ID (ADR-023 v1.4 specifies the collection, why the chat ID stays opaque, and — since the measurement below — why the sentinel is kept on grounds other than the race); the race loser re-reads the sentinel and returns the winner's `chat_id` as 200 OK (ADR-006 §4.1); group-size cap enforced by reading the chat document *inside* the transaction; membership mutation on a direct chat rejected; **every multi-document write retryable by its caller**, since the client library does not retry the `InvalidArgument` its own retry can provoke.
- **Measured before implementation (2026-08-10):** the premise ADR-023 v1.3 gave for the sentinel — *a query matching nothing locks nothing* — is **false on this path**. The naive alternative produced exactly one chat in 15/15 repetitions under both concurrency modes, against a query-deleted control that produced five every time, and the sentinel's `tx.Create` refused **zero** racers in 30 repetitions. The construction ships anyway; ADR-023 v1.4 states the surviving reasons. Evidence: `docs/artifacts/evidence/m1.3-direct-pair-{pessimistic,optimistic}.log`.
- **Gate:** concurrent direct-chat creation → exactly one, **with the refusing mechanism recorded** (a gate that accepts any refusal certifies a mechanism it may never fire — RTM-04 C7); group at the cap refuses the next member with the count still exact; membership mutation on a direct chat refused; owner cannot leave a group.
...
- **Delivers:** the in-process relay poller in Ingest — **claim (`FOR UPDATE SKIP LOCKED`) → produce Protobuf event with `acks=all` → mark `published_at`** (never before the ack; ADR-023 CI-2); Managed-Kafka producer with murmur2/`chat_id` partitioning (ADR-011) and Schema-Registry-encoded `MessagePersisted` (ADR-022 D1). **Plus the entity-event half moved here from M1.3 (2026-08-10):** Chat Mgmt publishes `chats.created` / `memberships.changed` **after** the Firestore write commits, at-least-once and deduped downstream by `event_id` (ADR-022 D4). It lands here because this is where the producer is built; the asymmetry between the two paths — transactional outbox for messages, post-commit publish for entities — is ADR-022 D4's and is now implemented in one place.
...
- **Gate:** message persisted → exactly one `MessagePersisted` on `messages.persisted`; induced crash mid-relay → event reappears, deduped downstream by `event_id`; **chat created and member added → `chats.created` / `memberships.changed` land, and only after the Firestore write committed** (the half moved from M1.3).
...
| ADR-016 | Chat lifecycle (§3.1–§3.3 superseded by ADR-023 v1.4) | M1.3 |
| ADR-017 | Test pyramid | M7.1–M7.4 |
| **ADR-021** | GCP substrate | M0.2, M2.1, M3.1, all infra |
| **ADR-022** | Analytics data lake | M0.3, M2.3 (both event paths), M6.1 |
| **ADR-023** | Two-store data model | M1.1, M1.2, M1.3, M2.1, M2.2, M4.1 |
...
| 2026-08-10 | **v2.7 (M1.3 premise measured; one scope move).** M1.3's headline invariant rested on a reason that was **measured false before the module's code was written**: ADR-023 v1.3 justified the `direct_chats` sentinel with *a query matching nothing locks nothing*, and on the direct-chat path the naive alternative produced exactly one chat in 15/15 repetitions under both concurrency modes, against a query-deleted control producing five every time. The sentinel ships regardless — **ADR-023 v1.4** states the three surviving grounds, led by the one that never depended on concurrency: it is the lookup ADR-006 §4.1's idempotent create reads. Two consequences for this plan. **(1) Entity-event publication moves M1.3 → M2.3**, because no Kafka producer exists yet and M2.3 builds the first one; M1.3's obligation to the critical path is membership reads, which is unchanged, so `M1.3 → M2.2` still holds. **(2) M1.3's gate now requires the *refusing mechanism* to be recorded**, not merely that exactly one chat survives — the sentinel refused zero racers in thirty repetitions, so a gate accepting any refusal would certify a mechanism it never fires (RTM-04 C7, reproduced prospectively). Also folded in from the same run: ADR-016 §3.1–§3.3 are superseded (two-phase group creation is a DynamoDB artefact), and every multi-document Firestore write must be caller-retryable, since the client library does not retry the `InvalidArgument` its own retry can provoke. | Alexis + Claude |
| 2026-07-27 | **v2.6 (M1.3 prerequisite specified).** Sharpened M1.3's direct-chat invariant, which said "Firestore transaction on the deterministic direct-pair doc id" — phrasing that reads as a deterministic *chat* ID, the option **ADR-023 v1.3** rejects because a chat ID travels through URLs, Kafka partition keys and lake rows, and deriving it from its members would publish the membership everywhere it appears. The invariant is `tx.Create` on the **`direct_chats/{min}__{max}` sentinel**, with the race loser re-reading it and returning the winner's `chat_id` as 200 OK (ADR-006 §4.1). No scope change; M1.3 is unblocked. | Alexis + Claude |
