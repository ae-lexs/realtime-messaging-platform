...
> [!NOTE]
> **Amended by [ADR-021](ADR-021.md)** (2026-07-24, AWS → GCP substrate migration). The Kafka provider changes from MSK Serverless to **GCP Managed Service for Apache Kafka**; the explicit-partitioning contract below is intact, and only the client authentication changes (MSK-IAM → SASL/OAUTHBEARER or mTLS). A **BigQuery Sink** consumer is added for the analytics data lake. The event-schema wire format (JSON below) is **superseded by [ADR-022](ADR-022.md)**: events are **Protobuf** via the managed Schema Registry, and the **FULL** compatibility policy of §5.1 is retained unchanged. ADR-022 preserves this ADR's envelope fields (`event_id` as ULID, `event_type`, `event_version`→`schema_version`, `producer_id`, `trace_id`) and drops only `partition_key` (always `chat_id`).
...
flowchart TB
    subgraph COMMIT["OFFSET COMMIT SEMANTICS"]
        ...
        subgraph PROCESS["Process Each Record"]
            P1["Record 100: Fanout attempted ✓"]
            P2["Record 101: Fanout attempted ✓"]
            P3["Record 102: Fanout attempted ✓"]
            P4["Record 103: Parse failed ✗"]
