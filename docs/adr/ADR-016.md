...
> [!IMPORTANT]
> **Amended for the GCP substrate by [ADR-023](ADR-023.md) v1.3 (2026-07-27, M1.3).** Every decision below stands; only its binding moved. §2.1's `direct_chat_index` table is the Firestore collection **`direct_chats/{min(user_a,user_b)}__{max(user_a,user_b)}`**, and its `attribute_not_exists(pair_key)` conditional put is a `tx.Create` on that deterministic path — the same guarantee, for the same reason, since a Firestore query that matches nothing locks nothing either. Two details to read with correction: the **separator is `__`, not `#`** (§2.1 justifies `#` by the ULID alphabet, and the implemented identifiers are UUIDs, which exclude both characters — `__` matches the neighbouring `memberships` collection), and §2.2's **five-item transaction is now four documents plus a cross-store gap**, because `chat_counters` is a Postgres row that cannot join a Firestore transaction. ADR-023 v1.3 has both.
