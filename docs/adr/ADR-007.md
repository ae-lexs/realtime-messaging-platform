...
> [!WARNING]
> **Superseded by [ADR-021](ADR-021.md)** (2026-07-24, AWS → GCP substrate migration) — the single-table DynamoDB model is replaced by a **two-store model**. The concrete schema (DDL, Firestore collections, access-pattern parity) is specified in **[ADR-023](ADR-023.md)**: PostgreSQL for the message/delivery write path (`chat_counters`, `messages`, `idempotency_keys`, `delivery_state`, `outbox`) and Firestore for identity/membership (`users`, `chats`, `memberships`, `sessions`). This record is retained unchanged.
