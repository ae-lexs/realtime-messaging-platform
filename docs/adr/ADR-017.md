...
> | **[CLIENT_PROTOCOL_CONTRACT.md](CLIENT_PROTOCOL_CONTRACT.md)** | 36 numbered client requirements (CL-\*, MS-\*, MR-\*, AK-\*, RS-\*, EH-\*), client state machine | Normative contract (MUST changes require ADR review) |
...
> **Full specification**: See **[CLIENT_PROTOCOL_CONTRACT.md](CLIENT_PROTOCOL_CONTRACT.md)** for the complete, versioned contract with all 36 requirements and the normative state machine diagram.
...
The contract defines a **normative** 7-state machine: `Disconnected → Connecting → Authenticating → Connected → Syncing → Ready → Closing`. Transitions outside this machine are bugs. The full diagram with per-state invariants is in [CLIENT_PROTOCOL_CONTRACT.md §7](CLIENT_PROTOCOL_CONTRACT.md#7-client-state-machine).
...
2. **Client Correctness**: The client protocol contract ([CLIENT_PROTOCOL_CONTRACT.md](CLIENT_PROTOCOL_CONTRACT.md)) formalizes the scattered checklists from ADR-005 and ADR-008 into a single, versioned, testable specification. Client developers have an unambiguous target.
...
7. **Evolvable Specification**: The document hierarchy (ADR → contract → test plans) enables the contract and test suite to evolve independently of the architectural decision. Adding a new invariant updates `CLIENT_PROTOCOL_CONTRACT.md` and `test/conformance/README.md`, not this ADR.
