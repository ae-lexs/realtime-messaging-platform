...
The events get a dedicated proto package so the wire contract is versioned independently of the gRPC service contracts. *(Implementation note, v1.2: the registry accepts only one message type per schema, so this listing ships as four files — `envelope.proto` plus one per event — each registering its imports as references. The definitions below are otherwise unchanged.)*

```proto
// proto/events/v1/ — one message per file (see the v1.2 changelog entry)
syntax = "proto3";
package events.v1;
...
message MembershipChanged {        // topic: memberships.changed
  ...
  string chat_id    = 2;
  string user_id    = 3;
  enum Change { CHANGE_UNSPECIFIED = 0; CHANGE_ADDED = 1; CHANGE_REMOVED = 2; CHANGE_ROLE_CHANGED = 3; }
...
|---|---|---|
| v1.2 | 2026-07-25 | **Implementation conformance (M0.3), no decision change.** The D1 snippet shows all four messages in one `events.proto`; the Managed Kafka schema registry rejects that with *"Too many message types specified in schema definition"*, so the file is split — `envelope.proto`, `message_persisted.proto`, `membership_changed.proto`, `chat_created.proto` — with each subject registering exactly one message and declaring its imports as references. Two further registry constraints, found by probing the live service: it resolves **no** imports itself (not even `google/protobuf/timestamp.proto`, which is now vendored under `proto/third_party` and registered as subject `wkt.timestamp`), and subject names may not begin with `google`. Consequence for the wire format: the message index is now always **0**, since each schema holds a single type. Field numbers, types, topics, subjects, and the metadata-only privacy scope are unchanged. |
| v1.1 | 2026-07-25 | **Implementation conformance (M0.3), no decision change.** `MembershipChanged.Change` values in the D1 snippet gained the enum-name prefix — `CHANGE_ADDED` / `CHANGE_REMOVED` / `CHANGE_ROLE_CHANGED` — because buf's `STANDARD` lint rule `ENUM_VALUE_PREFIX` (which this repo enforces in CI) requires it. Tag numbers, semantics, and the `CHANGE_UNSPECIFIED = 0` guarantee are unchanged. Recorded here rather than silently diverging in `proto/events/v1/events.proto`. |
