# Execution Plan — Realtime Messaging Platform (v2.6, GCP substrate)

- **Status**: Active
- **Created**: 2026-02-01
- **Re-split**: 2026-07-24 (v2.0 — modules → granular PRs on the GCP substrate)
- **Last amended**: 2026-07-27 (v2.6 — M1.3 prerequisite specified; see Document Revision History)
...
- **Key invariants:** at most one direct chat per pair under concurrency — `tx.Create` on the **`direct_chats/{min}__{max}` sentinel** inside the creation transaction, *not* a deterministic chat ID (ADR-023 v1.3 specifies the collection and why the chat ID stays opaque); the race loser re-reads the sentinel and returns the winner's `chat_id` as 200 OK (ADR-006 §4.1); group-size cap enforced; membership mutation on a direct chat rejected; event published only after the Firestore write commits.
