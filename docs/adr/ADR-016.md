...
> [!IMPORTANT]
> **Amended for the GCP substrate by [ADR-023](ADR-023.md), v1.3 (2026-07-27) and v1.4 (2026-08-10, M1.3).** §2.1's `direct_chat_index` table is the Firestore collection **`direct_chats/{min(user_a,user_b)}__{max(user_a,user_b)}`**, and its `attribute_not_exists(pair_key)` conditional put is a `tx.Create` on that deterministic path. Four corrections to read this document with:
>
> 1. **The reason given for that construction was wrong, and v1.3 stated it here too.** This banner claimed the same guarantee held "for the same reason, since a Firestore query that matches nothing locks nothing either." **Measured false** on this exact path (2026-08-10): the naive in-transaction query — §2.1's own rejected Option C — yielded exactly one chat in fifteen of fifteen repetitions under both concurrency modes, against a query-deleted control that yielded five every time. The sentinel is still built, on three grounds that do not depend on the race; **ADR-023 v1.4** carries them, and they are not this document's.
> 2. **The separator is `__`, not `#`** — §2.1 justifies `#` by the ULID alphabet, and the implemented identifiers are UUIDs, which exclude both characters, so `__` matches the neighbouring `memberships` collection.
> 3. **§2.2's five-item transaction is four documents plus a cross-store gap**, because `chat_counters` is a Postgres row that cannot join a Firestore transaction. M1.3 names the gap; M2.2 closes it with a self-creating counter.
> 4. **§3.1–§3.3 are superseded — see the marker at §3.** Two-phase group creation, its consistency window and its reconciler are artefacts of a DynamoDB limit the new substrate does not have.
...
> [!WARNING]
> **§3.1–§3.3 are superseded by [ADR-023](ADR-023.md) v1.4 and are retained for the record only.** Everything below follows from one premise stated in §3.1 — that a transaction is capped at **100 items**, so a 100-member group cannot be created atomically. That cap is DynamoDB's. Firestore documents no per-transaction document count; the limits that bind are the 10 MiB request size and the 270 s transaction time, against 1 chat + 100 memberships for a maximum group. **Group creation is therefore a single atomic transaction**, and with it go the two-phase split, the partial-membership window of §3.2, and the `member_count` reconciler of §3.3 — none of which is implemented.
>
> Two things below outlive the change and are still binding. **`member_count` is never used for authorization** (§3.2): it is display plus the group-size cap, and the membership check reads `memberships` directly. And the **owner-existence invariant is procedural** (§4.3), enforced by refusing the operations that would break it, because no store here can express "at least one member has role owner" as a constraint.
...
This record was accepted against DynamoDB and amended in place when the substrate changed. The body is **not** rewritten — the amendments are the banner at the top and the marker at §3, and this table says what moved and where the current statement lives. ADR-023 is authoritative wherever the two disagree.

| Version | Date | Changes |
|---|---|---|
| v1.2 | 2026-08-10 | **The direct-chat construction survives; the reason this record gave for it does not (M1.3).** The banner's v1.3 wording justified the Firestore sentinel "for the same reason, since a Firestore query that matches nothing locks nothing either" — a premise **measured false** on this path, where §2.1's own rejected Option C produced exactly one chat in fifteen of fifteen repetitions under both concurrency modes against a control that produced five every time. The sentinel stands on grounds [ADR-023](ADR-023.md) v1.4 states and this record never did. Separately, **§3.1–§3.3 are superseded**: two-phase creation, its consistency window and its `member_count` reconciler all descend from §3.1's 100-item transaction cap, which is DynamoDB's and not Firestore's — group creation is one atomic transaction, and none of that machinery is implemented. A §3 marker says so where a reader meets it, and names the two rules that outlive the change (`member_count` is never authorization; owner-existence is procedural). No decision in §2, §4–§10 is affected. |
| v1.1 | 2026-07-27 | **Re-bound to the GCP substrate by [ADR-023](ADR-023.md) v1.3.** `direct_chat_index` becomes `direct_chats/{min}__{max}` with `tx.Create` in place of the `attribute_not_exists` conditional put; the separator changes from `#` to `__` because the implemented identifiers are UUIDs rather than the ULIDs §2.1 assumed; and §2.2's five-item transaction becomes four Firestore documents plus a cross-store gap, since `chat_counters` is now a Postgres row. |
| v1.0 | 2026-02-01 | **Accepted.** Chat lifecycle and membership orchestration against DynamoDB: `direct_chat_index` for pair deduplication, two-phase group creation under the 100-item transaction cap, denormalized `member_count` as the group-size serialization point, direct-chat immutability enforced at the data layer, and the event-follows-commit contract. |
