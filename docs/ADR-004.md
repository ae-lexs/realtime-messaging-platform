...
ADR-001 established that messages within a chat must be totally ordered using a server-assigned monotonic `sequence` number. ADR-003 defined the `chat_counters` table as the storage mechanism for these counters. However, neither document specifies the precise mechanics of how sequence numbers are allocated under concurrent load, what consistency guarantees the allocation provides, or how the system behaves under failure conditions.
...
The `chat_counters` table stores one item per chat:

| Attribute | Type | Description |
|-----------|------|-------------|
| `chat_id` | String (PK) | Partition key; identifies the chat |
| `sequence_counter` | Number | Current highest allocated sequence (starts at 0) |
| `created_at` | String | ISO 8601 timestamp of counter creation |
| `updated_at` | String | ISO 8601 timestamp of last increment |

**Initial State**: When a chat is created, a counter item is created with `sequence_counter = 0`. The first message in the chat receives `sequence = 1`.
...
**Why Not Self-Healing?** If a counter were recreated via ADD starting at 1, it would allocate sequences that collide with existing persisted messages, violating the uniqueness invariant. This is a correctness-breaking failure mode that must be prevented, not recovered from automatically.
