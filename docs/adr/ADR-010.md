...
> [!NOTE]
> **Amended by [ADR-021](ADR-021.md)** (2026-07-24, AWS → GCP substrate migration). The Redis provider changes from ElastiCache to **Memorystore for Redis**. The Redis API, data structures, TTLs, ownership rules, and ephemeral-by-contract semantics below are unchanged.
